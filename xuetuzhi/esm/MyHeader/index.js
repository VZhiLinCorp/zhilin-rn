...
import { bgWhite, colorPrimary, center, px2dp, getBorder, headerHeight, getFlex, alignItemsC, COLOR_INFO, fontSizeMd, COLOR_WHITE, row, bgDanger, jc_c, spaceBtw, flexEnd, p_0, pl_xs } from 'zhilin-rn/styles';
...
export default class MyHeader extends React.Component {
    ...
    render() {
        ...
        return (
           <Header style={[{ marginTop: -1, paddingTop: px2dp(24) }, spaceBtw, alignItemsC, nb ? { borderBottomWidth: 0 } : getBorder('b'), headerHeight, headerPadding, { backgroundColor: bgColor ? bgColor : "#2F54EB" }]}>
                <View style={[getFlex(3), row, alignItemsC]}>{
                    ...
                    <Button transparent style={[getFlex(1), { marginLeft: isAndroid ? px2dp(-5) : 0, paddingLeft: 0 }]} onPress={onPress ? onPress : this._nav} dark>
                        {
                            leftText ? <Text style={[colorPrimary,pl_xs, { color: color ? color : COLOR_WHITE }]}>{leftText}</Text>
                                : <Icon type="Entypo" name="chevron-thin-left" style={[m_0, { color: color ? color : COLOR_WHITE }]} />
                        }
                ...
                <View style={[getFlex(10), row, jc_c]}>
                    {
                        renderBody ? renderBody() : <Title style={[fontSizeMd, { color: color ? color : COLOR_WHITE }]} >{this.props.title}</Title>
                    }
                </View>
